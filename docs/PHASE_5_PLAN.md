...
- [x] 每月 1 日本地 09:00 触发 owner 报表。
- [x] 失败重试后进入 failed 状态。
- [x] 生成的 Excel 包含报表字段和筛选条件摘要。
- [x] 未被分享的用户不能下载报表结果。
...
- [x] 非 admin 不能创建全局模板。
- [x] 从历史项目生成模板时不会复制敏感附件内容。
- [x] 使用模板新建项目会预填字段但生成新的业务编号。
- [x] 标签筛选结果受角色权限限制。
...
- [x] 样本不足时返回 `insufficient_sample`。
- [x] admin 与普通用户看到的样本集合不同。
- [x] 同类项目 P50/P90 计算正确。
- [x] 前端能展示周期、预算、逾期三个分析面板。
...
- [x] 无异常项目评分低于 30。
- [x] 超预算且多个逾期任务项目评分高于 70。
- [x] AI 关闭时仍返回确定性摘要。
- [x] AI 返回无效 JSON 时降级为规则摘要并记录审计。
