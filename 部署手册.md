...
- 非 Docker 部署：Python 3.11+、Node.js 20+、PostgreSQL 14+、Redis 6+。
...
适用于 PostgreSQL、Redis 已由服务器或运维平台独立安装的环境。

1. 安装系统依赖：

```bash
sudo apt-get update
sudo apt-get install -y python3.11 python3.11-venv nodejs npm postgresql redis-server nginx
```
...
CREATE USER project_mgmt WITH PASSWORD '<强密码>';
CREATE DATABASE project_mgmt OWNER project_mgmt;
\q
```

3. 配置 `.env`，数据库和 Redis 地址指向本机服务：

```env
DATABASE_URL=postgresql+asyncpg://project_mgmt:<强密码>@127.0.0.1:5432/project_mgmt
...
6. 启动后端、worker 和 beat。生产建议使用 systemd 管理，核心命令如下：

```bash
cd backend
. .venv/bin/activate
gunicorn app.main:create_app --factory -k uvicorn.workers.UvicornWorker -b 127.0.0.1:8000
celery -A app.tasks.celery_app.celery_app worker -l info
celery -A app.tasks.celery_app.celery_app beat -l info
```

7. 将 Nginx 指向 `frontend/dist`，并把 `/api/` 反向代理到 `127.0.0.1:8000`。上线后执行本文 3.5 健康检查。
